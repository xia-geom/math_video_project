...
\hypersetup{pdftitle={MAT0339 -- Série C, examen 2 -- corrige},pdfauthor={Document de travail pédagogique}}
...
\begin{enumerate}
\item \textbf{5 points.} \(10^2\cdot26\cdot25=65\,000\). Les deux lettres sont distinctes, contrairement aux chiffres.
\par{\small\textit{Barème : }Identification des choix/répétitions : 2; expression : 2; calcul : 1.}
\item \textbf{5 points.} Les rôles sont différents : \(8\cdot7=56\).
\par{\small\textit{Barème : }Rôle de l’ordre : 2; expression : 2; calcul : 1.}
\item \textbf{5 points.} Six lettres, deux E et deux T : \(6!/(2!2!)=180\).
...
\begin{enumerate}
\item \textbf{8 points.} Premier niveau : \(P(R_1)=3/5\), \(P(B_1)=2/5\). Après une rouge, les proportions sont \(2/4\) et \(2/4\); après une bleue, \(3/4\) et \(1/4\). Les chemins \(RR,RB,BR,BB\) ont respectivement les probabilités \[\frac3{10},\frac3{10},\frac3{10},\frac1{10}.\] Leur somme vaut \(1\).
\par{\small\textit{Barème : }Arbre et probabilités conditionnelles : 4; quatre produits : 4.}
\item \textbf{4 points.} Les chemins retenus sont disjoints : \(P=\frac3{10}+\frac1{10}=\frac25\).
\par{\small\textit{Barème : }Chemins pertinents : 2; somme : 2.}
\item \textbf{4 points.} Par définition du conditionnement, \(P(R_1\mid B_2)=\frac{P(R_1\cap B_2)}{P(B_2)}=\frac{3/10}{2/5}=\frac34\). Le dénominateur s’obtient en additionnant les chemins qui réalisent l’événement conditionnant.
\par{\small\textit{Barème : }Dénominateur conditionnel : 2; quotient : 2.}
\item \textbf{4 points.} Par symétrie des positions, \(P(R_2)=3/5\). Or \(P(R_2\mid R_1)=\frac24=\frac12\ne\frac35\). Les événements ne sont donc pas indépendants.
...
\begin{enumerate}
\item \textbf{4 points.} \(X\) suit une loi binomiale de paramètres \(n=6\) et \(p=0{,}5\). Chaque essai a deux issues, la probabilité de succès est constante, et les essais sont indépendants.
\par{\small\textit{Barème : }Deux issues, nombre fixe, probabilité constante et indépendance : 1 chacun.}
\item \textbf{4 points.} \(P(X=3)=\binom63(1/2)^6=5/16\). Le coefficient binomial compte les positions des succès.
\par{\small\textit{Barème : }Coefficient : 1; puissances : 2; résultat : 1.}
\item \textbf{3 points.} Le complément est « aucun succès » : \(P(X\ge1)=1-(1/2)^6=63/64\).
\par{\small\textit{Barème : }Complément : 1; calcul : 2.}
\item \textbf{4 points.} \(E(G)=8(1/2)-2(1/2)=3\) dollars par essai. C’est une moyenne théorique à long terme, non un gain garanti à chaque essai.
...
\begin{enumerate}
\item \textbf{3 points.} Par Pythagore, \(\|\mathbf u\|=2\sqrt5,\quad\|\mathbf v\|=3\).
\par{\small\textit{Barème : }Normes et racines : 3.}
\item \textbf{5 points.} \(\mathbf u\cdot\mathbf v=6\). Ainsi \(\cos\theta=1/\sqrt5\), et \(\theta\approx 63{,}43^\circ\), dans \([0,180^\circ]\).
\par{\small\textit{Barème : }Produit : 2; quotient de cosinus : 2; angle et mode degré : 1.}
\item \textbf{7 points.} \[\operatorname{proj}_{\mathbf v}\mathbf u=\frac{\mathbf u\cdot\mathbf v}{\mathbf v\cdot\mathbf v}\mathbf v=\frac69(0,3)=(0,2).\] Le résidu est \((4,0)\). Son produit scalaire avec \(\mathbf v\) vaut zéro; la projection est bien parallèle à \(\mathbf v\).
...
\begin{enumerate}
\item \textbf{6 points.} Par produit ligne-colonne, \[AB=\begin{pmatrix}4 & 1\\3 & 1\end{pmatrix},\qquad BA=\begin{pmatrix}3 & 1\\5 & 2\end{pmatrix}.\] Les matrices ne commutent pas.
\par{\small\textit{Barème : }Deux produits : 5; comparaison : 1.}
\item \textbf{6 points.} \(\det A=1\ne0\). La formule de l’inverse donne \[A^{-1}=\begin{pmatrix}1 & -1\\-2 & 3\end{pmatrix}.\] Le produit \(AA^{-1}\) est \(I_2\).
\par{\small\textit{Barème : }Déterminant : 2; inverse : 3; produit identité : 1.}
\item \textbf{8 points.} En appliquant l’inverse, \[\mathbf x=A^{-1}\begin{pmatrix}11\\8\end{pmatrix}=\begin{pmatrix}3\\2\end{pmatrix}.\] Vérification : \(3\cdot3+1\cdot2=11\) et \(2\cdot3+1\cdot2=8\).
...
\begin{enumerate}
\item \textbf{8 points.} La substitution donne \(3+2t=7\), donc \(t=2\) et le point \((4,1,4)\). Le produit de la normale par le directeur vaut \((1,-1,1)\cdot(1,1,2)=2\ne0\), donc l’intersection est unique.
\par{\small\textit{Barème : }Substitution : 3; paramètre et point : 3; unicité : 2.}
\item \textbf{7 points.} La somme des deux premières équations impose \(2x+2z=6\). Si \(k\ne6\), le système est incompatible. Si \(k=6\), la troisième équation est redondante; la différence des deux premières fixe \(y=(5-1)/2\). Avec \(z=s\), toutes les solutions sont \((x,y,z)=(3-s,2,s)\), \(s\in\mathbb R\). Il n’existe aucun cas de solution unique.
