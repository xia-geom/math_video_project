...
\hypersetup{pdftitle={MAT0339 -- Série A, examen 3 -- sujet},pdfauthor={Document de travail pédagogique}}
...
Ressource 1 & 1 & 2 & 12\\
Ressource 2 & 2 & 1 & 12\\
Bénéfice (\$ par unité) & 30 & 40 & ---\\\bottomrule
...
\begin{enumerate}
\item \textbf{(3 points)} Convertir exactement l’angle \(\theta=225^\circ\) en radians.
\item \textbf{(4 points)} Donner les valeurs exactes de \(\sin\theta\) et \(\cos\theta\), en justifiant les signes par le quadrant.
\item \textbf{(4 points)} Un secteur circulaire a un rayon de 6 cm et un angle au centre \(\alpha=2\pi/3\) radians. Calculer la longueur de son arc.
...
\begin{enumerate}
\item \textbf{(6 points)} Dans un triangle \(ABC\), les côtés \(a=7\) cm et \(b=9\) cm sont opposés aux angles \(A\) et \(B\), et l’angle compris vaut \(C=60^\circ\). Faire un schéma étiqueté, choisir une loi et déterminer exactement \(c\).
...
\begin{enumerate}
\item \textbf{(7 points)} Résoudre \(\sin x=-1/2\) sur \([0,2\pi[\). Justifier que la liste est complète.
\item \textbf{(8 points)} Résoudre \(\cos(2x)=1/2\) sur \([0,2\pi[\). Donner des valeurs exactes.
\end{enumerate}
\vspace{3mm}\textit{Démarche et réponse :}\par\vfill
...
\begin{enumerate}
\item \textbf{(9 points)} Une cabine de grande roue varie entre 2 m et 26 m, avec une période de 40 s. Au temps \(t=0\), elle est au point le plus bas. Construire une formule en sinus ou cosinus. Identifier amplitude, médiane et période, puis vérifier les données caractéristiques.
\item \textbf{(6 points)} Trouver tous les instants où la cabine est à 20 m pendant \([0,40[\) secondes.
\end{enumerate}
\vspace{3mm}\textit{Démarche et réponse :}\par\vfill
...
\begin{enumerate}
\item \textbf{(6 points)} Résoudre \((x^2-9)/(x-3)=8\), en conservant le domaine initial.
\item \textbf{(6 points)} Déterminer la réciproque de \(g(x)=2x+1\), de \(\mathbb R\) vers \(\mathbb R\), et vérifier les deux compositions.
\item \textbf{(8 points)} Trois essais indépendants ont chacun une probabilité de succès de 0,2. Calculer la probabilité d’au moins un succès et expliquer le rôle de l’indépendance.
\end{enumerate}
\vspace{3mm}\textit{Démarche et réponse :}\par\vfill
\end{document}
