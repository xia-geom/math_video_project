...
from manim import (
    BLACK, BLUE_D, DOWN, LEFT, RIGHT, UP, WHITE,
...
def card(title, body, x=0, y=0.2, width=3.6, height=1.7):
    ...
    for text in (heading, content):
        if text.width > width - 0.35 or text.height > height - 0.55:
            raise ValueError("Card text does not fit; shorten it rather than shrink it")
...
class ConversationArchiveIntroEN(Scene):
    ...
    def beat_opening(self):
        cards = VGroup(card("A conversation", "An idea", -4.2),
                       ...
                       card("A later note", "A new perspective", 4.2))
    ...
    def beat_evidence(self):
        source = card("Original text", '"I postponed the summer build\nafter the budget review."', -3.5, width=5.5, height=2.25)
        record = card("E0004", "User statement\nExact source span\nOriginal wording retained", 3.5, width=5.5, height=2.25)
    ...
    def project_cards(self):
        return VGroup(card("E0001", "Plan Orchard", -4.2, -0.1),
                      card("E0002", "Build the prototype", 0, -0.1),
                      card("E0005", "Restart Orchard", 4.2, -0.1))
    ...
    def beat_connections(self):
        ...
        self.play(FadeIn(project), Create(links), run_time=1.1)
        self.play(FadeIn(label("PROPOSED CONNECTION  /  SEPARATE ENTRIES", 23, MUTED).move_to([0, -1.55, 0])), run_time=0.5)
    ...
    def beat_questions(self):
        ...
        self.play(FadeIn(label("EXAMPLE CARD  /  ONLY DISPLAYED REFERENCES ARE IN SCOPE", 19, MUTED).move_to([0, -1.8, 0])), run_time=0.5)
    ...
    def beat_corrections(self):
        ...
        self.play(Create(links), run_time=1.1)
    ...
    def beat_architecture(self):
        ...
        records = card("Structured records", "Entries\nRules and evidence", -4.25, width=3.7, height=1.9)
        ...
        self.play(FadeIn(badge), FadeIn(records), run_time=0.7)
        self.play(Create(arrows[0]), FadeIn(index), run_time=0.7)
        self.play(Create(arrows[1]), FadeIn(views), run_time=0.7)
    ...
    def beat_start(self):
        ...
        self.play(FadeIn(route), run_time=0.7)
